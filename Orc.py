...
class OrcStateSeeking(State):
    def __init__(self, orc):
        ...
        self.path_graph = self.orc.world.paths[
            randint(0, len(self.orc.world.paths) - 1)
        ]
    ...
    def check_conditions(self):
        ...
        if (self.orc.position - self.orc.move_target.position).length() < 8:
            ...
            if self.current_connection < self.path_length:
                self.orc.move_target.position = self.path[
    ...
    def entry_actions(self):
        ...
        if self.path_length > 0:
            ...
            self.orc.move_target.position = self.path_graph.nodes[
                self.orc.base.target_node_index
            ].position
...
class OrcStateAttacking(State):
    ...
    def check_conditions(self):
        ...
        if (
            self.orc.target.position - self.orc.position
        ).length() > self.orc.min_target_distance:
            return "seeking"
